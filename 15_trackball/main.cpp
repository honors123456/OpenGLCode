...
void click_callback(GLFWwindow* window, int button, int action, int mods) {
    if (button == GLFW_MOUSE_BUTTON_1) {
        ...
        mapToSphere(glm::vec2(keyx / SCR_WIDTH, (SCR_HEIGHT-keyy) / SCR_HEIGHT) * 2.0f - 1.0f,lastpos);
     }
}
...
void mouse_callback(GLFWwindow* window, double xposIn, double yposIn)
{
    ...
    float keyy = SCR_HEIGHT-static_cast<float>(yposIn);
    if (leftkey) {
       glm::vec3 curpos = glm::vec3(0.0f);
        mapToSphere(glm::vec2(keyx / SCR_WIDTH, keyy / SCR_HEIGHT) * 2.0f - 1.0f, curpos);
        if(lastpos==curpos) return;
        glm::vec3 axis_w = glm::cross(lastpos, curpos);
        glm::vec3 axis_l = glm::vec3(glm::inverse(model) * glm::vec4(axis_w, 1));
        float angle =  glm::length(lastpos - curpos);
        model = glm::rotate(model, angle, axis_l);
        lastpos = curpos;
        ...
        // model = glm::rotate(r,angle,axis_l);
